...
export class Gamepad extends EventEmitter implements IGamepad {
    ...
    constructor(throttle: boolean = false) {
        super()
        ...
        this.axisSensitivity = this.throttle ? 0.7 : 0.2
    ...
    updateLoop() {
        ...
        setTimeout(() => {
            ...
            pollingGP.buttons.forEach((button: any, index: number) => {
                button.type = "button"
            ...
            pollingGP.axes.forEach((axis: any, index: number) => {
                if (index <= 3) {
                    let activeAxis: IGamepadEvent = null

                    if (
                        axis > this.axisSensitivity ||
                        axis < -this.axisSensitivity
                    ) {
                        activeAxis = {
                            type: "axis",
            ...
            this.loopRequest = window.requestAnimationFrame(
